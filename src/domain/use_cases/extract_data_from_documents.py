...
class ExtractAuctionDataFromDocumentsUseCase:
    def __init__(self, document_extractor: DocumentExtractor, pdfdocument_result_extractor: DocumentExtractor):
    ...
    def execute(self, document_paths: list[str]) -> Optional[AuctionDataExtracted]:
        ...
        try:
            ...
            result = AuctionDataExtracted(
                **document,
                sold_batches=extracted_data['sold_batches'],
                unsold_batches=extracted_data['unsold_batches']
            )
    ...
    def _extract_properties_from_notice(self, document_paths: list[str]) -> dict[str, str]:
    ...
    def _extract_auction_result(self, document_paths: list[str]) -> dict[str, str]:
        # Extracts the auction data from ata document
        path = next((path for path in document_paths if re.search(r'(resultado).*(cpf.*cnpj)', path.split('/')[-1].lower()) is not None), None)
        if not path:
