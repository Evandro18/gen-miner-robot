...
class SaveAuctionItemFilesUseCase:
    def __init__(self, file_repository: SaveFileRepository, hashing: HashString) -> None:
        self._file_repository = file_repository
        self._hashing = hashing

    async def save(self, auction_item: AuctionItemEntity) -> tuple[list[str], list[str]]:
    ...
    async def _download_list(self, urls: list[str], folder: str, filename_extractor: Callable[[str], str]) -> list[str]:
        urls_result: list[str] = []
        for img_url in urls:
            url = await self._file_repository.save(img_url, folder, extract_filename=filename_extractor)
            if url is not None:
                urls_result.append(url)
        
        return [url for url in urls_result if isinstance(url, str)]