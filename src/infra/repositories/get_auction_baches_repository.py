        ...
        for i in range(1, size):
            ...
            for j in range(1, city_size):
                ...
                for k in range(1, period_size):
                    ...
                    # Pegar a paginação
                    pages = await page.query_selector_all('#resultadoVitrine #paginacao a:not([id])')
                    ...
                    for l in range(1, len(pages)):
                        ...
                        items = await page.query_selector_all('#resultadoVitrine2 .resultado-busca-preco span')
                        await page.wait_for_selector('#resultadoVitrine2 .resultado-busca-preco span')
                        ...
                        for i in range(1, len(items)):
                            ...
                            jewelry = await extract_data_from_modal(page, request_interceptor_state)
                            close_btn = await page.query_selector('#modalResultadoDetalhe .close span')
...
async def extract_data_from_modal(page: Page, request_interceptor_state: InterceptorState):
    fields = await page.query_selector_all('#modalResultadoDetalhe .modal-resultado-busca-campos span')
    await page.wait_for_timeout(1000)
    ...
    valueSelector = await page.query_selector('#modalResultadoDetalhe #modalResultadoValorLance span')
    ...
    return {
        ...
        'value': value,
        'images': images,
        'documents': [url_edital_action, url_catalog_action]
    }
